...
class LpSetup : public CosemObject {
public:
    uint16_t class_id() const override { return static_cast<uint16_t>(CosemInterface::IecLocalPortSetup); }
    uint8_t version() const override { return 0; }
    std::string class_name() const override { return "LPSetup"; }

    uint32_t baud_rate_ = 9600; uint8_t parity_ = 0; uint8_t stop_bits_ = 1;

    std::vector<AttributeAccess> get_attributes() const override {
        return {AttributeAccess{1, "logical_name", true}, AttributeAccess{2, "baud_rate", true}, AttributeAccess{3, "parity", true}, AttributeAccess{4, "stop_bits", true}};
    }
