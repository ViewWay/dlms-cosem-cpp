...
class LorawanSetup : public CosemObject {
public:
    uint16_t class_id() const override { return static_cast<uint16_t>(CosemInterface::LoRaWANSetup); }
    uint8_t version() const override { return 0; }
    std::string class_name() const override { return "LoRaWANSetup"; }

    std::string dev_eui_; std::string app_eui_; std::string app_key_;

    std::vector<AttributeAccess> get_attributes() const override {
        return {AttributeAccess{1, "logical_name", true}, AttributeAccess{2, "dev_eui", true}, AttributeAccess{3, "app_eui", true}, AttributeAccess{4, "app_key", true}};
    }
